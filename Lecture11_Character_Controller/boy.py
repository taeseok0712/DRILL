...
# Boy Event
RIGHT_DOWN, LEFT_DOWN, RIGHT_UP, LEFT_UP , TIME_UP, SHIFT_DOWN, SHIFT_UP  = range(7)
...
class DashState:
    def enter(boy, event):
        ...
        boy.timer = 1000
    def exit(boy, event):
        if boy.velocity == -3:boy.velocity = -1
        elif boy.velocity == 3: boy.velocity = 1
        pass
    def do(boy):
        boy.frame = (boy.frame + 1) % 8
        boy.timer -= 5
...
class Boy:
    ...
    def draw(self):
        ...
        debug_print('Velocity :' + str(self.velocity) + ' state:' + str(self.cur_state)  )
        pass
